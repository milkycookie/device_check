...
/* This code is written by non-developer for non-developers according to KISS principle. 
Gtfo with your refactoring skills. */
...
function checkBannerWidthHeight(){

    var tryWidthOne = document.body.clientWidth;
    var tryWidthTwo = document.body.offsetWidth;

    clientInfoJson.banner_width = "0";
    clientInfoJson.banner_height = document.body.clientHeight;

    if (tryWidthOne > 0){
        clientInfoJson.banner_width = tryWidthOne;
    }
    if (tryWidthTwo > 0){
        clientInfoJson.banner_width = tryWidthTwo;
    }
    
}

checkBannerWidthHeight();
...
// REST API consists of two listeners:
// "clientInfoUrl" is for collecting client info when banner is loaded and temporarily for clicks

var clientInfoUrl = "http://82.202.221.106/client_info";

// When banner is shown we need to send clientInfoJson JSON to database 
// for the purpose of collecting all advertising data even without clicks
...
// Simple banner actions — clicks. Send it to "clientInfoUrl" — for the first iteration creating new line in db
// We need to merge lines before getting the data from db

document.getElementById('banner').addEventListener('click', function () {
    clientInfoJson.click = "1";
    sendClientInfoJson();
});
